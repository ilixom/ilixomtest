...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total ilixomtest balance of the wallet\n"
            "  \"darksend_balance\": xxxxxx, (numeric) the anonymized ilixomtest balance of the wallet\n"
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"ilixomtestaddress\"\n"
            "\nReturn information about the given ilixomtest address.\n"
            "\nArguments:\n"
            "1. \"ilixomtestaddress\"     (string, required) The ilixomtest address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"ilixomtestaddress\", (string) The ilixomtest address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Ilixomtest address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are ilixomtest addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) ilixomtest address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"ilixomtestaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"ilixomtestaddress\"  (string, required) The ilixomtest address to use for the signature.\n"
